...
summary(lm(yrkemp ~ e, data = cova_table))
...
```{r balance}
# identify cova columns
cova_columns <- ftp_ar %>% 
  select(where(~ 
                 {
                   column_label <- attributes(.)$label
                   
                   str_detect(column_label, 'cova')
  ...
  # select(where(~ str_detect(sapply(ftp_ar, function(x) attributes(x)$label), 'cova')))
  #   
  #   
  #   
  #   attributes(.)$label, 'cova')))




  
# select(where(~ str_detect(attributes(.)$label, 'cova')))
# 
